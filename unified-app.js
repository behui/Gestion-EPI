...
window.changerStatut = function(lockerId, nouveauStatut) {
    ...
    if (locker) {
        ...
        // 📱 ÉCRITURE TAG NFC OBLIGATOIRE quand on passe EN ATTENTE → OCCUPÉ
        ...
        // 📱 EFFACEMENT TAG NFC OBLIGATOIRE quand on passe INACTIF/OCCUPÉ → LIBRE/ACTIF
...
async function ecrireTagOccupation(locker) {
    ...
    try {
        ...
        const validation = validerTailleMessage(message);
        if (!validation.valide) {
            afficherToast(`❌ Données trop longues: ${validation.taille} bytes (max 100)`, 'error', 6000);
            return;
        }
        ...
        afficherToast(
            `✅ Tag NFC écrit (${validation.taille}/100 bytes)\n${message}`,
            'success',
            5000
        );
...
function afficherInfoTag(data, serialNumber) {
    // Parser les données (format: "Nom Prenom|M:REF|T:REF")
    const parts = data.split('|');
    ...
    modal.innerHTML = `
        <div style="text-align:center;padding:20px;">
            ...
            <h2 style="color:#667eea;margin:0 0 20px 0;font-size:1.5em;">Tag NFC Identifié</h2>
    ...
    afficherToast(`✅ Tag identifié: ${nomComplet}`, 'success', 4000);
}
...
async function effacerTagNFC(casierId) {
    ...
    try {
        ...
        // Écrire un message vide ou un message de reset
        const messageReset = `LIBRE:${casierId}`;
        
        const validation = validerTailleMessage(messageReset);
        if (!validation.valide) {
            afficherToast('❌ Erreur validation message', 'error', 3000);
            return;
        }
        ...
        afficherToast(
            `✅ Tag NFC effacé et réinitialisé\n\nCasier ${casierId} prêt pour la personne suivante`,
            'success',
            5000
        );
        
        console.log(`🗑️ Tag NFC effacé: "${messageReset}" (${validation.taille} bytes)`);
        
    } catch (error) {
        console.error('❌ Erreur effacement NFC:', error);
        afficherToast('❌ Erreur effacement NFC: ' + error.message, 'error', 5000);
    }
}

console.log('✅ Gestion Masques Ventilés Digit 2.0 chargé avec succès !');
console.log('📱 Système NFC prêt (< 100 bytes garantis)');
console.log('📱 Format 1: Écriture après validation occupation');
console.log('📱 Format 2: Lecture pour identification propriétaire');

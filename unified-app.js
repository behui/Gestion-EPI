...
function renderUtilisateurs() {
    ...
    let html = '<div style="background:white;border-radius:20px;padding:25px;box-shadow:0 10px 40px rgba(0,0,0,0.10);">';
    html += '<h2 style="color:#667eea;font-size:1.8em;font-weight:800;margin:0 0 15px 0;">👤 Gestion des Utilisateurs</h2>';
    html += '<p style="color:#64748b;margin-bottom:25px;">Cycle complet : <strong>EN ATTENTE</strong> → valider popup → <strong>OCCUPÉ</strong> → matériel revient → <strong>INACTIF</strong> → nettoyé → <strong>LIBRE</strong></p>';
    
    if (utilisateurs.length === 0) {
        html += '<p style="text-align:center;color:#9ca3af;padding:40px;">Aucun utilisateur assigné</p>';
    } else {
        ...
        utilisateurs.forEach(user => {
            ...
            html += `
                <div style="background:${c.bg};border-left:6px solid ${c.border};padding:20px;border-radius:12px;">
                    <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:15px;flex-wrap:wrap;gap:15px;">
                        <div style="flex:1;">
                            <div style="font-size:1.3em;font-weight:800;color:#1f2937;margin-bottom:5px;">${user.prenom} ${user.nom}</div>
                            <div style="font-size:0.9em;color:#64748b;margin-bottom:8px;">Casier <strong>${user.id}</strong> - ${user.platform}</div>
                            <div style="display:inline-block;padding:6px 14px;border-radius:20px;font-size:0.8em;font-weight:700;background:${c.border};color:white;">${user.statut}</div>
                        </div>
                        <div style="text-align:right;">
                            ${user.refMasque ? `<div style="font-size:0.85em;color:#64748b;margin-bottom:4px;">🎭 ${user.refMasque}</div>` : ''}
                            ${user.refTuyau ? `<div style="font-size:0.85em;color:#64748b;margin-bottom:4px;">🔧 ${user.refTuyau}</div>` : ''}
                            ${user.dateAssignation ? `<div style="font-size:0.75em;color:#9ca3af;margin-top:6px;">📅 ${new Date(user.dateAssignation).toLocaleDateString('fr-FR')}</div>` : ''}
                        </div>
                    </div>`;
            ...
            if (user.statut === 'EN ATTENTE') {
                html += `
                    <button onclick="openModalValidationOccupation('${user.id}')" style="width:100%;padding:12px;border:none;background:#ef4444;color:white;font-size:1em;font-weight:700;border-radius:10px;cursor:pointer;transition:all 0.3s;">
                        🔴 VALIDER → OCCUPÉ
                    </button>
                `;
            }
            // OCCUPÉ → INACTIF
                ...
                html += `
                    <button onclick="changerStatut('${user.id}', 'INACTIF')" style="width:100%;padding:12px;border:none;background:#64748b;color:white;font-size:1em;font-weight:700;border-radius:10px;cursor:pointer;transition:all 0.3s;">
                        ⚫ METTRE EN INACTIF
                    </button>
                `;
            }
            // INACTIF → LIBRE
                ...
                html += `
                    <button onclick="changerStatut('${user.id}', 'LIBRE')" style="width:100%;padding:12px;border:none;background:#10b981;color:white;font-size:1em;font-weight:700;border-radius:10px;cursor:pointer;transition:all 0.3s;">
                        🟢 LIBÉRER LE CASIER
                    </button>
                `;
            }

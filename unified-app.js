...
window.scannerNFC = async function() {
    ...
    if ('NDEFReader' in window) {
        try {
            ...
            scanModal.innerHTML = `
                <div style="background:white;border-radius:20px;padding:40px;text-align:center;max-width:400px;animation:pulse 2s infinite;">
                    ...
                    <div style="background:#f0f9ff;padding:15px;border-radius:10px;margin-bottom:20px;">
                        ...
                        <div style="font-size:0.9em;color:#1e40af;">En attente du signal NFC...</div>
                    </div>
                    <button onclick="this.closest('div[style*=fixed]').remove()" style="width:100%;padding:12px;border:2px solid #e2e8f0;background:white;color:#64748b;font-size:1em;font-weight:700;border-radius:12px;cursor:pointer;">
                        Annuler
                    </button>
                </div>
            `;
            ...
            ndef.addEventListener("reading", ({ message }) => {
                ...
                // Remplir les champs
                if (marque) document.getElementById('new-marque').value = marque;
                if (reference) document.getElementById('new-ref').value = reference;
                if (numeroAxens) document.getElementById('new-modele').value = numeroAxens;
            ...
            alert('❌ Erreur NFC: ' + error.message);
        }
    } else {
        // Simulation pour les navigateurs sans NFC (pour tester)
        const simulation = confirm('⚠️ NFC non disponible sur ce navigateur.\n\nVoulez-vous utiliser le mode SIMULATION pour tester ?');

...
const STORAGE_KEYS = {
    ...
    historiqueAchats: 'ppve_historique_achats'
};
...
window.ouvrirModeleRationalisation = function() {
    ...
    // Calculer les coûts moyens RÉELS depuis l'historique des achats
    const historique = JSON.parse(localStorage.getItem(STORAGE_KEYS.historiqueAchats)) || [];
    const coutMoyen = { masque: 150, tuyau: 80, moteur: 350, batterie: 200 }; // Prix par défaut
    
    // Si des achats ont été enregistrés, utiliser les coûts réels
    ['masques', 'tuyaux', 'moteurs', 'batteries'].forEach(cat => {
        const achats = historique.filter(h => h.categorie === cat);
        if (achats.length > 0) {
            const totalQuantite = achats.reduce((sum, a) => sum + a.quantite, 0);
            const totalCout = achats.reduce((sum, a) => sum + a.coutTotal, 0);
            coutMoyen[cat.slice(0, -1)] = Math.round(totalCout / totalQuantite); // Coût moyen réel
        }
    });
...
function renderEquipActions() {
    document.getElementById('equip-actions').innerHTML = `
        ...
        <button onclick="ouvrirHistoriqueAchats()" style="padding:12px 24px;border:none;background:linear-gradient(135deg,#f59e0b 0%,#d97706 100%);color:white;font-size:1em;font-weight:700;border-radius:12px;cursor:pointer;transition:all 0.3s;box-shadow:0 4px 15px rgba(245,158,11,0.3);">
            🛒 Historique Achats
        </button>
...
window.ouvrirHistoriqueAchats = function() {
    const historique = JSON.parse(localStorage.getItem(STORAGE_KEYS.historiqueAchats)) || [];
    
    // Calculer les statistiques
    const stats = {
        masques: { quantite: 0, cout: 0 },
        tuyaux: { quantite: 0, cout: 0 },
        moteurs: { quantite: 0, cout: 0 },
        batteries: { quantite: 0, cout: 0 }
    };
    
    historique.forEach(achat => {
        if (stats[achat.categorie]) {
            stats[achat.categorie].quantite += achat.quantite;
            stats[achat.categorie].cout += achat.coutTotal;
        }
    });
    
    const coutTotal = Object.values(stats).reduce((sum, s) => sum + s.cout, 0);
    const quantiteTotal = Object.values(stats).reduce((sum, s) => sum + s.quantite, 0);
    
    let html = `
        <div style="max-height:85vh;overflow-y:auto;padding-right:10px;">
            <!-- EN-TÊTE -->
            <div style="background:linear-gradient(135deg,#f59e0b 0%,#d97706 100%);color:white;padding:25px;border-radius:12px;margin-bottom:25px;">
                <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:15px;flex-wrap:wrap;gap:10px;">
                    <div>
                        <h2 style="margin:0 0 10px 0;font-size:1.8em;font-weight:800;">🛒 HISTORIQUE DES ACHATS</h2>
                        <div style="font-size:0.95em;opacity:0.9;">Suivi des acquisitions et calcul des coûts moyens réels</div>
                    </div>
                    <button onclick="closeModal()" style="background:rgba(255,255,255,0.2);color:white;border:none;border-radius:8px;padding:10px 20px;font-weight:700;cursor:pointer;">✕</button>
                </div>
                ...
                <div style="background:rgba(255,255,255,0.2);padding:15px;border-radius:10px;display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;text-align:center;">
                    <div>
                        <div style="font-size:1.8em;font-weight:800;">${historique.length}</div>
                        <div style="font-size:0.8em;opacity:0.9;">Achats enregistrés</div>
                    </div>
                    <div>
                        <div style="font-size:1.8em;font-weight:800;">${quantiteTotal}</div>
                        <div style="font-size:0.8em;opacity:0.9;">Équipements achetés</div>
                    </div>
                    <div>
                        <div style="font-size:1.8em;font-weight:800;">${coutTotal.toLocaleString('fr-FR')}€</div>
                        <div style="font-size:0.8em;opacity:0.9;">Budget total</div>
                    </div>
                </div>
            </div>
            
            <!-- BOUTON NOUVEL ACHAT -->
            <button onclick="ajouterNouvelAchat()" style="width:100%;padding:16px;border:none;background:linear-gradient(135deg,#10b981 0%,#059669 100%);color:white;font-size:1em;font-weight:800;border-radius:12px;cursor:pointer;margin-bottom:25px;box-shadow:0 4px 15px rgba(16,185,129,0.3);">
            ...
            <div style="background:white;border:2px solid #e2e8f0;border-radius:12px;padding:20px;margin-bottom:25px;">
                <div style="font-weight:800;color:#1f2937;margin-bottom:15px;font-size:1.1em;">📊 COÛTS MOYENS RÉELS</div>
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;">
    `;
    
    const categories = {
        masques: { icon: '🎭', label: 'Masques', color: '#3b82f6' },
        tuyaux: { icon: '🔧', label: 'Tuyaux', color: '#10b981' },
        moteurs: { icon: '⚙️', label: 'Moteurs', color: '#f59e0b' },
        batteries: { icon: '🔋', label: 'Batteries', color: '#ef4444' }
    };
    
    Object.entries(categories).forEach(([key, info]) => {
        const stat = stats[key];
        const coutMoyen = stat.quantite > 0 ? (stat.cout / stat.quantite).toFixed(0) : 0;
        
        html += `
            <div style="background:linear-gradient(135deg,${info.color}15 0%,${info.color}25 100%);border:2px solid ${info.color};padding:15px;border-radius:10px;text-align:center;">
                <div style="font-size:2em;margin-bottom:8px;">${info.icon}</div>
                <div style="font-size:0.85em;font-weight:700;color:#64748b;margin-bottom:8px;">${info.label}</div>
                <div style="font-size:1.5em;font-weight:800;color:${info.color};">${coutMoyen}€</div>
                <div style="font-size:0.7em;color:#9ca3af;margin-top:4px;">${stat.quantite} achat(s)</div>
            </div>
        `;
    });
    ...
    html += `
                ...
                <div style="margin-top:15px;padding:12px;background:#f0f9ff;border-radius:8px;font-size:0.85em;color:#1e40af;">
                    💡 Ces coûts moyens alimentent automatiquement le modèle d'optimisation des commandes
                </div>
            </div>
            
            <!-- HISTORIQUE DES ACHATS -->
            <div style="background:white;border:2px solid #e2e8f0;border-radius:12px;padding:20px;">
                <div style="font-weight:800;color:#1f2937;margin-bottom:15px;font-size:1.1em;">📋 HISTORIQUE DÉTAILLÉ</div>
    `;
    
    if (historique.length === 0) {
        html += '<p style="text-align:center;color:#9ca3af;padding:40px;">Aucun achat enregistré</p>';
    } else {
        // Trier par date décroissante
        const historiqueTrié = [...historique].sort((a, b) => new Date(b.date) - new Date(a.date));
        
        html += '<div style="display:grid;gap:12px;">';
        historiqueTrié.forEach((achat, index) => {
            const cat = categories[achat.categorie];
            const dateAchat = new Date(achat.date).toLocaleDateString('fr-FR', { 
                year: 'numeric', 
                month: 'long', 
                day: 'numeric' 
            });
            const coutUnitaire = achat.coutTotal / achat.quantite;
            
            html += `
                <div style="background:#f9fafb;border-left:5px solid ${cat.color};padding:15px;border-radius:8px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:15px;">
                    <div style="flex:1;min-width:200px;">
                        <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
                            <span style="font-size:1.5em;">${cat.icon}</span>
                            <div>
                                <div style="font-weight:800;color:#1f2937;font-size:0.95em;">${cat.label}</div>
                                <div style="font-size:0.75em;color:#64748b;">${dateAchat}</div>
                            </div>
                        </div>
                        ${achat.reference ? `<div style="font-size:0.8em;color:#667eea;margin-top:4px;">Réf: ${achat.reference}</div>` : ''}
                        ${achat.fournisseur ? `<div style="font-size:0.8em;color:#64748b;margin-top:2px;">🏢 ${achat.fournisseur}</div>` : ''}
                    </div>
                    <div style="text-align:right;">
                        <div style="font-size:1.3em;font-weight:800;color:${cat.color};">${achat.coutTotal.toLocaleString('fr-FR')}€</div>
                        <div style="font-size:0.8em;color:#64748b;margin-top:4px;">${achat.quantite} × ${coutUnitaire.toFixed(0)}€</div>
                        <button onclick="supprimerAchat(${index})" style="margin-top:8px;padding:6px 12px;border:none;background:#fee2e2;color:#991b1b;font-size:0.75em;font-weight:700;border-radius:6px;cursor:pointer;">
                            🗑️ Supprimer
                        </button>
                    </div>
                </div>
...
window.ajouterNouvelAchat = function() {
    let html = `
        <div style="max-height:80vh;overflow-y:auto;padding-right:10px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
                <h3 style="color:#f59e0b;font-size:1.5em;font-weight:800;margin:0;">🛒 Nouvel Achat</h3>
                <button onclick="ouvrirHistoriqueAchats()" style="background:#ef4444;color:white;border:none;border-radius:8px;padding:8px 16px;font-weight:700;cursor:pointer;">✕ Annuler</button>
            </div>
            
            <form id="form-achat" style="display:grid;gap:15px;">
                <div>
                    <label style="display:block;font-weight:700;color:#1f2937;margin-bottom:8px;">📅 Date d'achat *</label>
                    <input type="date" id="achat-date" required style="width:100%;padding:12px;border:2px solid #e2e8f0;border-radius:8px;font-size:1em;" value="${new Date().toISOString().split('T')[0]}">
                </div>
                
                <div>
                    <label style="display:block;font-weight:700;color:#1f2937;margin-bottom:8px;">📦 Catégorie *</label>
                    <select id="achat-categorie" required style="width:100%;padding:12px;border:2px solid #e2e8f0;border-radius:8px;font-size:1em;">
                        <option value="">-- Sélectionner --</option>
                        <option value="masques">🎭 Masques</option>
                        <option value="tuyaux">🔧 Tuyaux</option>
                        <option value="moteurs">⚙️ Moteurs</option>
                        <option value="batteries">🔋 Batteries</option>
                    </select>
                </div>
                
                <div>
                    <label style="display:block;font-weight:700;color:#1f2937;margin-bottom:8px;">🔖 Référence (optionnel)</label>
                    <input type="text" id="achat-reference" placeholder="Ex: REF-2024-001" style="width:100%;padding:12px;border:2px solid #e2e8f0;border-radius:8px;font-size:1em;">
                </div>
                
                <div>
                    <label style="display:block;font-weight:700;color:#1f2937;margin-bottom:8px;">🏢 Fournisseur (optionnel)</label>
                    <input type="text" id="achat-fournisseur" placeholder="Ex: Fournisseur EPI SA" style="width:100%;padding:12px;border:2px solid #e2e8f0;border-radius:8px;font-size:1em;">
                </div>
                
                <div>
                    <label style="display:block;font-weight:700;color:#1f2937;margin-bottom:8px;">🔢 Quantité *</label>
                    <input type="number" id="achat-quantite" required min="1" placeholder="Ex: 10" style="width:100%;padding:12px;border:2px solid #e2e8f0;border-radius:8px;font-size:1em;">
                </div>
                
                <div>
                    <label style="display:block;font-weight:700;color:#1f2937;margin-bottom:8px;">💰 Coût total (€) *</label>
                    <input type="number" id="achat-cout" required min="0" step="0.01" placeholder="Ex: 1500.00" style="width:100%;padding:12px;border:2px solid #e2e8f0;border-radius:8px;font-size:1em;">
                </div>
                
                <div style="background:#f0f9ff;border:2px solid #3b82f6;border-radius:10px;padding:15px;">
                    <div style="font-weight:700;color:#1e40af;margin-bottom:8px;">💡 Conseil</div>
                    <div style="font-size:0.85em;color:#64748b;line-height:1.5;">
                        Ces données permettent de calculer les coûts moyens réels et d'optimiser les prévisions budgétaires futures.
                    </div>
                </div>
                
                <button type="submit" style="width:100%;padding:14px;border:none;background:linear-gradient(135deg,#10b981 0%,#059669 100%);color:white;font-size:1em;font-weight:800;border-radius:10px;cursor:pointer;box-shadow:0 4px 15px rgba(16,185,129,0.3);">
                    ✅ ENREGISTRER L'ACHAT
                </button>
            </form>
        </div>
    ...
    document.getElementById('form-achat').addEventListener('submit', (e) => {
        ...
        const achat = {
            ...
            quantite: parseInt(document.getElementById('achat-quantite').value),
            coutTotal: parseFloat(document.getElementById('achat-cout').value)
        };
        
        const historique = JSON.parse(localStorage.getItem(STORAGE_KEYS.historiqueAchats)) || [];
        historique.push(achat);
        localStorage.setItem(STORAGE_KEYS.historiqueAchats, JSON.stringify(historique));
        
        alert(`✅ Achat enregistré !\n\n${achat.quantite} équipement(s) pour ${achat.coutTotal}€\nCoût unitaire: ${(achat.coutTotal / achat.quantite).toFixed(2)}€`);
        ouvrirHistoriqueAchats();
    });
};

window.supprimerAchat = function(index) {
    if (!confirm('Voulez-vous vraiment supprimer cet achat de l\'historique ?')) return;
